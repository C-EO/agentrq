...
import { createStorageStore } from './extensions/storage.js'
...
function buildExtensionRuntime() {
  ...
  /**
   * Settings an extension defines the shape of, as opposed to the manifest's.
   *
   * Its own file rather than a section of `extensions-config.json`: that one is
   * values AgentRQ validated against a declared schema and can render, and this
   * one is opaque blobs it never looks inside. Keeping them apart means the
   * settings screen can go on reading a file it understands.
   */
  const storageStore = createStorageStore({
    store: jsonStore('extensions-storage.json'),
    vault: {
      available: () => safeStorage.isEncryptionAvailable(),
      encrypt: (text) => safeStorage.encryptString(text).toString('base64'),
      decrypt: (blob) => safeStorage.decryptString(Buffer.from(blob, 'base64')),
